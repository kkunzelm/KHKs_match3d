...
public class Match3d_withFiducialMarkersAndICPv2_1 implements PlugIn, DialogListener {
	...
	private static final String[] interpolationMethod = {"nearest neigbor", "bilinear", "bicubic",
			"bicubic (Bob Dougherty's Style)" // File: CubicFloatProcessore.java needed!!
												// Bob Dougherty 6/19/2008. Modified from FloatProcesor by Wayne
												// Rasband.
	...
	// neu ab 2014
	List<Vector3d> mesh1 = new ArrayList<>();
	List<Vector3d> mesh2 = new ArrayList<>();
	...
	private PolygonRoi polyRoiSource; // convention in my code:
	...
	private PolygonRoi polyRoiTarget;
	...
	private List<Vector3d> landmarks1 = new ArrayList<>();
	private List<Vector3d> landmarks2 = new ArrayList<>();
	...
	public static Matrix getInverseTransformationMatrix(double[][] transformationMatrix, boolean printInfos) {
		...
		// with R being a rotation and t being a translation is a combined
		// transformation
		...
		// nebenbei: es gilt immer... erst in Ursprung verschieben, dann rotieren, dann
		// weiter translatieren
		...
		// Inverse of a sequence of transformations is the composition of the inverses
		// of each transformation in reverse order.
	...
	public void run(String arg) {
		...
		// in medicine corresponding points are sometimes called fiducial marks or
		// ficudial markers
		...
		/*
		 * // for debugging - testing my matrix multiplication double[] cLA = new
		 * double[3]; cLA[0] = centroidLandmarks2[0]; cLA[1] = centroidLandmarks2[1];
		 ...
		 * c.print(10,2);
		 *
		 * double[] translationM = new double[3]; //translation calculated based on
		 * matrix operations translationM[0] = centroidLandmarks1[0] - c.get(0,0);
		 * translationM[1] = centroidLandmarks1[1] - c.get(1,0); translationM[2] =
		 * centroidLandmarks1[2] - c.get(2,0);
		 * System.out.println("Translation von Matrix Mult: " + translationM[0] + "  " +
		 * translationM[1] + "  " + translationM[2]);
		 *
		...
		// applyGeneralForwardTransformation3D(imp2, impForwardTransformedImage,
		// transformationMatrix);

		/*
		 * ImagePlus impTransformedImage = makeTarget(imp1);
		 * impTransformedImage.setTitle("Transformed Landmark based");
		 * applyGeneralTransformation3D(imp1, imp2, impTransformedImage,
		 * transformationMatrix);
		 */
		...
		// applyGeneralForwardTransformation3D(imp2, impForwardTransformedImageICP,
		// tMat);
	...
	public void getCorrespondingPointBruteForce(ImagePlus imp1, ImagePlus imp2) {
		...
		for (i = 0; i < vectorArray1.length; i++) {
			for (j = 0; j < vectorArray2.length; j++) {
				...
				double dist = (vectorArray1[i].x - vectorArray2[j].x) * (vectorArray1[i].x - vectorArray2[j].x)
						+ (vectorArray1[i].y - vectorArray2[j].y) * (vectorArray1[i].y - vectorArray2[j].y)
						+ (vectorArray1[i].z - vectorArray2[j].z) * (vectorArray1[i].z - vectorArray2[j].z);
				if (minDistance > dist) {
					minDistance = dist;
...
//			System.out.println("New" + vert.x + "\t" + vert.y + "\t" + vert.z);
	...
	public List<Vector3d> getCorrespondingPointListFromImageData(ImagePlus imp) {
		...
		for (int i = 0; i < ip.getHeight(); i = i + step) {
			for (int j = 0; j < ip.getWidth(); j = j + step) {
				...
				if (ip.getf(j, i) > 0.001) { // quick and dirty: remove negative numbers, zero and those close to
												// zero... think for a more generic solution
					vert.z = ip.getf(j, i);
				}
	 ...
	 * Decomposition
	 *
	 * @param relativeLandmarks1
	 *            Landmark list number 1
	 * @param relativeLandmarks2
	 *            Landmark list number 2
	...
	private double[][] correlationMatrixK(List<Vector3d> relativeLandmarks1, List<Vector3d> relativeLandmarks2) {
		...
		/*
		 * this method implements formula (3) of Kanatani the weight is set to "1". It
		 * is a scaling factor only. Formula (3):
		 *
		 ...
		 * K
		 *
		 * KHK Added: 19.1.13 from: http://nghiaho.com/?page_id=671 Pay close attention
		 * to the transpose symbol. It’s doing a multiplication between 2 matrices where
		 * the dimensions effectively are, 3×1 and 1×3, respectively. The ordering of
		 * the multiplication is also important, doing it the other way will find a
		 * rotation from B to A instead.
		 *
		 *
		 */
	...
	private Matrix svd(double[][] correlationMatrixK) {
		...
		/*
		 * To perform the singular value decomposition svd the package Jama is needed As
		 * soon as we have the correlation matrix, the next step in the algorithm as
		 * described by Kanatani is to decompose the correlation matrix (see formulae 12
		 * and 13)
		 *
		 * It is important to mention that Kanatani writes: K = VAU^T In Jama there is a
		 * svd.jama file which uses a slightly different syntax A = USV^T Please
		 * concentrate not to mix the meaning of U and V Kanatini's U will be the V of
		 * SVD to stay in the logic of Jama we write for formula (13)
		 *
		 * (1 0 0 ) R = U * (0 1 0 )*V^T formula 13 later called intermedResult (0 0
		 * det(UV^T) )
		 *
		 * Just to mention a few details:
		 *
		 * Kanatani writes in formula (4) that to minimize the distance between the two
		 * pointsets for the given correlation matrix (formula 3), the trace(R^T*K)
		 * musst be maximized in formula (13) he states - and proves too - when the
		 * trace(R^T*K) is maximized
		 *
		 * What we do is, we use K make a SVD and get U and V Using U and V we can
		 * calculate R with formula 13 R is our final result for the rotation
		 *
	...
	private double[] computeTranslation(double[] centroid1, double[] centroid2, double[][] rotationMatrixR) {
		...
		// Important: to calculate the translation, the centroid of the target image,
		// which will be transformed, has to be rotated!!!
		// T = Sc - Tc*R
		// Sc = Source image center, Tc = Target image center

		// Lets assume now that we have a matrix R. When P is multiplied by R it
		// transforms P to PT. Considering what we know about matrix multiplication lets
		// see how we can re-write a point-matrix multiplication and isolate the
	...
	private ImagePlus makeTarget(ImagePlus imp1) {
		// ip1 = baseline image which will not be transformed but we use it as a
		// template for the transformed image.
	...
	public void applyGeneralForwardTransformation3D(ImagePlus imp2, ImagePlus impForwardTransformedImage,
		...
		for (int v = 0; v < h; v++) {
			for (int u = 0; u < w; u++) {
				 ...
				 * System.out.println(u + ", " + v + ", " + ip2.getf(u,v) + " = " + x + "/" +
				 * pointTransformed.get(0,0) + " " + y + "/" +pointTransformed.get(1,0) + " " +
				 * pointTransformed.get(2,0)); }
				 */
	...
	/**
	 * DESCRIPTION: Takes two input datasets and the location parameters as a
	 * transformation matrix in homogeneous coordinates and computes the differences
	 * per pixel between the rotated and translated patterns. The difference is
	...
	private void applyGeneralTransformation3D(ImagePlus imp1, ImagePlus imp2, ImagePlus differenceImage,
		...
		// target-to-source mapping as described for 2d in Burger and Burge
		// (http://imagingbook.com/)

		// impTransformedImage is a blank image plus template same size as the "target"
		// image (here ip1)
		...
		for (int v = 0; v < h; v++) { // rows of the image
			for (int u = 0; u < w; u++) { // columns of the image
				...
				Matrix pointTarget = new Matrix(4, 1); // I make a matrix from the array to use available JAMA matrix
														// multiplication later
				Matrix pointInvTransformed; // = new Matrix(4,1) for the point after the inverse transformation;

				// see Peter Neugebauer Match3d: Feinjustierung von Tiefenbildern zur Vermessung
				// von kleinen Verfomungen, Diplomarbeit 1991.
				// The baseline image is mapped with the inverse transformation matrix close to
				// the follow-up image (= source): f(u,x) -> f'(x,y)
				// At x,y there exists usually no value for the source data, therefore this
				// value has to be interpolated.
				// The difference is calculated and mapped using the transformation from x,y
				// back to u,v.
				// In our case we do not really need to apply the transformation as we just have
				// to use u,v which is the same after all.
				// The difference is now in the target image (= differenceImage) which has the
				// same coordinates as the baseline image.
				...
				// from JAMA API: matrixA.times(Matrix B) Linear algebraic matrix
				// multiplication, A * B
		...
		// Tipp: man kann mit Threshold im 32 bit float Bild das gleiche machen, wie bei
		// Wolfram mit clip-Histogram
		...
		ce.stretchHistogram(ipTransformedImage, 1.0); // ist eine Art Histogram Threshold. Es werden die obersten und
														// untersten Pixel entfernt.
														// unklar ist, ob 1% von oben und 1% von unten oder 1%/2 von
														// oben und 1%/2 von unten???
		double min = ipTransformedImage.getMin();
		double max = ipTransformedImage.getMax();
		ipTransformedImage.setMinAndMax(min, max);
...
//		}
	...
	// added for future reference: in case we want to read wavefront.obj files
	// directly.
...
// Components can then share an object that implements an event handling
// interface
// or extends an abstract adapter class, containing the code to be executed when
...
// This type of code is often considered messy and the inner class variations
// are considered to be better in all regards.
...
class WriteMatrix {
	 ...
	 * 
	 * @param d
	 *            the {@code double} value to be represented.
	 ...
	 *         represents that integer value, without decimals and preceding decimal
	 *         separator symbol. The string returned when {@code Double.isNaN(d)}
	 *         yields {@code true} can be specified with method . Similarly, the
	 *         string returned when {@code Double.isInfinite(d)} yields {@code true}
	 *         can be specified with method . The returned string is "0" if the
