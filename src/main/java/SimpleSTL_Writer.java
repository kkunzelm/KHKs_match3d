...
public class SimpleSTL_Writer implements PlugInFilter {
	...
	private Calibration cal;
        ...
        int width;
	int height;
        Pnt3d validPoints[];        // Pnt3d = Point class defined at the end of this class file, similar to vecmath.Vector3d etc.
        Pnt3d ret[]; 
	...
	public void write(DataOutputStream out) throws IOException {
		...
		//IJ.showStatus("Exporting as text...");
                ...
                validPoints = readVerticesForObjectFile();
		...
		for (int p=0; p<validPoints.length; p = p+3)
		{
                         ...
                         pw.println("facet normal "+a.x+" "+a.y+" "+a.z);
                           ...
                           // todo check: if scaleX and Y are different, the it could be that I have to swap scaleX and scaleY here --- see comment line 260ff
   ...
   Pnt3d[] readVerticesForObjectFile() {
       ...
       // this should give a triangulated surface
        ...
        // KHK todo: vector() is an obsolete or deprecated collection -> use ArrayList instead!
        Vector temp = new Vector();     // wie ein Array nur ohne vordefinierte Länge
        
        // started to modify code to ArrayList, but postponed it for the moment.
/*        List<Vector3f> vertexArrayList = new ArrayList<Vector3f>();
        Vector3f vertex; 
...
*/        
	...
	float tempX, tempY, tempZ;
	    ...
	    ret = new Pnt3d[temp.size()];
            temp.toArray(ret); // to preserve type Pnt3d
	    return ret;
   }

   Pnt3d dotProduct(Pnt3d P1, Pnt3d P2, Pnt3d P3){
       
       // for Paraview normalization not necessary
       
       /* dot product a = b x c  -> Wikipedia.org
        *         (a_x)
        *     a = (a_y) , same for b and c
        *         (a_z)    
        * 
        *     a_x = b_y c_z - b_z c_y  
        *     a_y = b_z c_x - b_x c_z  
       ...
       */
        /*
        double norm_b = Math.sqrt((b.x * b.x) + (b.y * b.y) + (b.z * b.z));
        double norm_c = Math.sqrt((c.x * c.x) + (c.y * c.y) + (c.z * c.z));
       ...
       // bei dieser Regel gehen die beiden Vektoren, mit denen die Normale berechnet wird 
       // beide vom Punkt P1 aus. Die Richtung der Normale ergibt ich nach der Rechten Handregel:
       // Daumen zeigt von P1 nach P2, Zeigefinger zeigt von P1 nach P3, Normale zeigt in Richtung Mittelfinger
   ...
   public class Pnt3d {
	...
	public Pnt3d (float x, float y, float z){
		this.x = x;  
		this.y = y;
                this.z = z;
