...
/**
 * Does target-to-source transformation of range data.
 * Data must be on a regular grid for this implementation.
 *  
 * @author kkunzelm
 * 
 *
...
public class KHKsRangeDataTransformJ {
    ...
    // KHK todo: klären - Übergabe der Transformationsmatix in Zukunft als JAMA matrix oder als double[][] Array
    // KHK todo: überlegen, ob analog zu TransformJ eine Option zur Erweiterung der Bildgröße eingeplant werden sollte??
    public static void applyT2sTransformationRangeData(ImagePlus imp, double[][] transformationMatrix, int interpolationMethod){
        ...
        impTransformedImage = NewImage.createFloatImage("Transformed t2s Img "+interpolationMethod,w,h,1,NewImage.FILL_BLACK);
        ...
        // the inverse transformation is applied to the normal vector pointing in the target frame of reference in z-direction
        
        Matrix directionAsMatrix = new Matrix(3,1);  // I make a matrix from the array to use available JAMA matrix multiplication later
        ...
        Matrix normOfPlane = new Matrix(4,1);  // I make a matrix from the array to use available JAMA matrix multiplication later
    ...
    public static Vec3 transformPoint (Vec3 vector, Matrix transformation){
...
private static double bicubicInterpolationBobDoughertyStyle(ImagePlus imp, Matrix pointInvTransformed){
    ...
    // efficient implementation of matrix inverse for affine transform, not used here so far
