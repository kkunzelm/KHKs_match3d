...
// from Wikipedia:
// Local inner classes are often used in Java to define callbacks for GUI code. Components can then share an object that
// implements an event handling interface or extends an abstract adapter class, containing the code to be executed when
// a given event is triggered.
// This avoids a large monolithic actionPerformed(ActionEvent) method with multiple if-else branches to identify the
// source of the event. This type of code is often considered messy and the inner class variations are considered to be
