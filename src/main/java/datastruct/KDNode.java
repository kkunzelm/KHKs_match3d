...
public class KDNode {
    ...
    // considered
    public static final double VACUUM_RATIO = 0.5;

    // Maximum number of points in a leaf node
    public static final int MAX_LEAVES = 8;


    protected int dimension; // 0=x, 1=y, 2=z, (-1=no cut)
    protected double cutoff;
    protected KDNode firstChild;
    protected KDNode secondChild;

    protected Point3d points[];

    protected int firstLeaf;
    protected int lastLeaf;
	...
	if ((last - first + 1) <= MAX_LEAVES) {
	    ...
	    if (cx0 > x0) {System.err.println("X min out of bounds!");}
	    if (cx1 < x1) {System.err.println("X max out of bounds!");}
	    if (cy0 > y0) {System.err.println("Y min out of bounds!");}
	    if (cy1 < y1) {System.err.println("Y max out of bounds!");}
	    if (cz0 > z0) {System.err.println("Z min out of bounds!");}
	    if (cz1 < z1) {System.err.println("Z max out of bounds!");}
	    // two cutoff-checks per dimension
	    int which; double val;
	    which = 0; val = (x0 - cx0) / (cx1 - cx0);
	    if ((cx1 - x1)/(cx1 - cx0) > val) {which = 1; val= (cx1 - x1)/(cx1 - cx0);}
	    if ((y0 - cy0)/(cy1 - cy0) > val) {which = 2; val= (y0 - cy0)/(cy1 - cy0);}
	    if ((cy1 - y1)/(cy1 - cy0) > val) {which = 3; val= (cy1 - y1)/(cy1 - cy0);}
	    if ((z0 - cz0)/(cz1 - cz0) > val) {which = 4; val= (z0 - cz0)/(cz1 - cz0);}
	    if ((cz1 - z1)/(cz1 - cz0) > val) {which = 5; val= (cz1 - z1)/(cz1 - cz0);}
	    if (val > VACUUM_RATIO) {
		// vacuum cutoff!
    ...
    protected void sort(int first, int last, int dim) {
	...
	*/
	...
	//System.out.println("     (sorting from "+first+" to "+last+" by "+dim+")");

	Point3d temp;

	if (first >= last) return; // no sort needed, 0 or 1 elements
	if (first+1 == last) {
	    // sorting two elements
	    boolean sw = false;
	    switch(dim) {
	    case 0: if (points[first].x > points[last].x) sw = true; break;
	    case 1: if (points[first].y > points[last].y) sw = true; break;
	    case 2: if (points[first].z > points[last].z) sw = true; break;
	    }
	    if (sw) {
		// switch both
		temp = points[last];
	...
	// partioning: first..pivot-1, pivot, pivot+1..last                 Pivot = Achse, Drehpunkt, vermutl. Mittenelement

	int left, right;
	double pivot_value = 0.0;
	int pivot;
	pivot = (first+last)/2; // middle point as pivot
	// move to front
	temp = points[pivot];   //temp: type Point3d
	points[pivot] = points[first];
	points[first] = temp;	    
	switch(dim) {
	case 0: pivot_value = points[first].x; break;
	case 1: pivot_value = points[first].y; break;
	case 2: pivot_value = points[first].z; break;
	}
	pivot = left = first;
	right = last;
	while (left < right) {
	    ...
	    switch(dim) {
	    case 0:
		while ((left < last) && (points[left].x <= pivot_value)) left++;
		while ((right > first) && (points[right].x >= pivot_value)) right--;
		break;
	    case 1:
		while ((left < last) && (points[left].y <= pivot_value)) left++;
		while ((right > first) && (points[right].y >= pivot_value)) right--;
		break;
	    case 2:
		while ((left < last) && (points[left].z <= pivot_value)) left++;
		while ((right > first) && (points[right].z >= pivot_value)) right--;
		break;
	    }
	...
	// right is partition number;

	sort(first, right-1, dim);
	sort(right+1, last, dim);
	...
	case 0: 
	    for (int i=first; i<=last; i++) {
		if (points[i].x < val) {
		    System.err.println("Inversion while sorting!");
		}
		val = points[i].x;
	    }
	    break;
...
// 	    }
	    ...
	    switch (dimension) {
	    case 0: splitSide = cutoff - point.x; break;
	    case 1: splitSide = cutoff - point.y; break;
	    case 2: splitSide = cutoff - point.z; break;
	    }
		...
		switch(dimension) {
		case 0:dx = (cutoff - point.x) * (cutoff - point.x);break;
		case 1:dy = (cutoff - point.y) * (cutoff - point.y);break;
		case 2:dz = (cutoff - point.z) * (cutoff - point.z);break;
		}
		d = dx + dy + dz;
		if ((d < distance) && (secondChild != null)) {
		    // look only if child is closer than radius
		    p2 = secondChild.findNearest(point, distance,
						 dx, dy, dz);
		}
		if (p2 >= 0) p = p2;
	    } else {
		...
		if ((d < distance) && (firstChild != null)) {
		    // look only if child is closer than radius
		    p2 = firstChild.findNearest(point, distance,
						 dx, dy, dz);
		}
		if (p2 >= 0) p = p2;
	    }
    ...
    /* KHK: 
     * I had to replace the classic Quicksort implementation with
     * this variation because of stack overflow errors.
     * 
     * 
    ...
    public static void sort(Point3d[] points, int dim) {
        sort(points, 0, points.length - 1);
	switch(dim) {
	    case 0:
		assert isSortedX(points); 
		break;
	    case 1:
		assert isSortedY(points); 
		break;
	    case 2:
		assert isSortedZ(points); 
	    }
    }
    ...
    */
    ...
    private void sortKH(int left, int right, int dim) { 
        ...
        //System.out.println("from KD Node     (sorting from "+left+" to "+right+" by "+dim+")" + "right<=left: " + (right <= left));
    ...
    ***********************************************************************/
    ...
    private static boolean isSortedZ(Point3d[] points, int left, int right) {
        for (int i = left + 1; i <= right; i++) {
            if (points[i].z < points[i-1].z) return false;
        }
        return true;
    }
